...
knitr::opts_knit$set(root.dir = "~/Documents/GitHub/methods2/classes/data")
...
clothing_advice <- function(temperature) {
  clothing <- ifelse(temperature > 25, "T-shirt", 
                     ifelse(temperature > 15, "light jacket", "warm coat"))
  return(paste("For temperature =", temperature, "wear a", clothing))
}

temperature <- 20
clothing_advice(temperature)
...
```{r}
?rbinom
#num of samples, from how many total, probability of our wanted observation
...
basketball <- function() {
  ...
  while (miss < 2) {
    basketballs <- rbinom(n = 1, size = 1, prob = 0.6)
    if (basketballs == 1 & miss != 1) {
      hits <- hits + 1
    }
    if (basketballs == 0) {
      miss <- miss + 1
  ...
  return(list(shots = shots, miss = miss, hits = hits))
}
...
```{r}
# Initialize an empty vector to store the number of shots taken in each iteration
shots_taken <- numeric(1000)
shots_hits <- numeric(1000)

# Run the basketball simulation 1000 times
for (i in 1:1000) {
  result <- basketball()
  shots_taken[i] <- result$shots
  shots_hits[i] <- result$hits
}

# Calculate the mean and standard deviation of the number of shots taken
mean_shots <- mean(shots_taken)
sd_shots <- sd(shots_taken)
...
```{r}
# your code here
basketball_df <- data.frame(
  shots = shots_taken,
  proportion = shots_hits / shots_taken
)

ggplot(data = basketball_df, aes(x = shots, y = proportion))+
  geom_point()+
  labs(x = "Number of Shots", y = "Proportion of Hits") +
  theme_minimal()
...
```{r}
# your code here
basketball_10shots <- function(){
  player <- rbinom(1, 10, 0.4)  # Simulate the number of shots made by the player in one game
  if(player == 3){  # Check if the player made exactly 3 shots
...
# Initialize a variable to store the number of times exactly 3 shots are made out of 10, 
# initialized as 0
count <- 0

# Loop the function 10,000 times
for (i in 1:10000){
  count <- count + basketball_10shots()
}

# Calculate the simulated probability
simulated_probability <- count / 10000
...
```{r}
# your code here
variables <- numeric(1000)
for (i in 1:1000){
  variables[i] <- sum(rbinom(20, 1, 0.5))
}


hist(variables, breaks = 30, probability = TRUE, 
     main = "Histogram of 1000 Simulations")

# Overlay normal density curve
mu <- 20 * 0.5  # mean of the sum of 20 uniform(0,1) variables
sigma <- sqrt(20 * (1/12))  # standard deviation of the sum
...
for (i in 1:1000){
  ...
  difference[i] <- men - women
}

hist(difference)
mean_sim <- mean(difference)
sd_sim <- sd(difference)

mean_sim
sd_sim

mean_exact <- 69.1 - 63.7
sd_exact <- sqrt((2.9^2/100) + (2.7^2/100))
...
simulated_effects <- rnorm(1000, true_eff, sd_est)

result <- t.test(simulated_effects)
confidence_interval <- result$conf.int
confidence_interval #the mean’s computed confidence interval is 0.09603985 0.10259530
mean(simulated_effects)
sd(simulated_effects)
```
...
```{r}
# your code here
replications <- numeric(10000)
for (i in 1:1000){
  replications[i] <- mean(rnorm(127, 0.10, 0.17))
}
...
count <- 0
for (i in 1:1000){
  values <- rnorm(127, 0.10, 0.17)
  replications[i] <- mean(values)
  result <- t.test(values)
  conf <- result$conf.int
  if (conf[1]<= replications[i] & replications[i] <= conf[2]){
    count = count + 1
  }
}
...
for (i in 1:1000){
  ...
  if (result$p.value < 0.05){
    significant[[length(significant) + 1]] <- mean(values)
  }
}

significant <- unlist(significant)
